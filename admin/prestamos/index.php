    ...
    <main class="app-main">
            ...
            <div class="app-content-header">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <h3 class="mb-0">Listado de prestamos</h3>
                        </div>
                    </div>
                    <hr>
                    <!--end::Row-->
                    ...
                    <table id="tablaLibros" class="table table-hover">
                        ...
                        <tbody class="table-group-divider">
                            <?php
                                $query = $pdo->prepare('SELECT * FROM prestamos');
                                $query->execute();
                                ...
                                foreach($prestamos as $prestamo){
                                    ...
                                    $fecha_devolucion = $prestamo['fyh_devolucion'];
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php $query_libro = $pdo->prepare('SELECT titulo FROM tb_libros WHERE id_libro = ?');
                                                $query_libro->execute([$id_libro]);
                                                $libros = $query_libro->fetchAll(PDO::FETCH_ASSOC);
                                                foreach($libros as $libro){$titulo_libro=$libro['titulo'];}
                                                echo $titulo_libro;
                                            ?>
                                        </td>
                                        <td>
                                            <?php $query_usuarioss = $pdo->prepare('SELECT * FROM tb_usuarios WHERE id_usuario = ?');
                                                $query_usuarioss->execute([$id_user]);
                                                $usuarioss = $query_usuarioss->fetchAll(PDO::FETCH_ASSOC);
                                                foreach($usuarioss as $userr){
                                                    $nombre=$userr['nombre_usuario'];
                                                    $carggo=$userr['cargo'];
                                                    $curso=$userr['curso'];
                                                    $paralelo=$userr['paralelo'];
                                                ?>
